...
namespace DRRealState.Core.Application.Services
{
    public class EstateServices : GenericServices<SaveEstateViewModel, EstateViewModel, Estate>, IEstateServices
    {
        ...
        public List<EstateViewModel> FilterAgentHouses(List<EstateViewModel> vm,FilterEstateViewModel filter) {
            ...
            if (filter.EstateType != null && filter.EstateType.Count > 0)
            {
                foreach (int Id in filter.EstateType)
                {
                    var es = vm.Where(x => x.PropertyTypeId == Id).ToList();
            ...
            if (filter.BedQuantity != null
                ...
                && filter.MinimumPrice != null)
            {
                return vm.Where(x => x.BedRoomQuantity == filter.BedQuantity
                && x.BathroomQuantity == filter.BathQuantity &&
                (x.Price >= filter.MinimumPrice && x.Price <= filter.MaximumPrice)).ToList();
            }
            if (filter.BedQuantity != null
                ...
                && filter.MinimumPrice == null)
            {
                return vm.Where(x => x.BedRoomQuantity == filter.BedQuantity).ToList();
            }
            if (filter.BedQuantity == null
                && filter.BathQuantity != null
                && filter.MaximumPrice == null
                && filter.MinimumPrice == null)
            {
                return vm.Where(x => x.BathroomQuantity == filter.BathQuantity).ToList();
            }
            if (filter.BedQuantity == null
                && filter.BathQuantity == null
                && filter.MaximumPrice != null
                && filter.MinimumPrice == null)
            {
                return vm.Where(x => x.Price <= filter.MaximumPrice).ToList();
            }
            if (filter.BedQuantity == null
                && filter.BathQuantity == null
                && filter.MaximumPrice == null
                && filter.MinimumPrice != null)
            {
                return vm.Where(x => x.Price >= filter.MinimumPrice).ToList();
            }
            #endregion
            ...
            if (filter.BedQuantity != null
                ...
                && filter.MinimumPrice == null)
            {
                return vm.Where(x => x.BedRoomQuantity == filter.BedQuantity && x.Price<=filter.MaximumPrice).ToList();
            }
            if (filter.BedQuantity != null
                && filter.BathQuantity == null
                && filter.MaximumPrice == null
                && filter.MinimumPrice != null)
            {
                return vm.Where(x => x.BedRoomQuantity == filter.BedQuantity && x.Price >= filter.MinimumPrice).ToList();
            }
            if (filter.BedQuantity != null
                && filter.BathQuantity == null
                && filter.MaximumPrice != null
                && filter.MinimumPrice != null)
            {
                return vm.Where(x => x.BedRoomQuantity == filter.BedQuantity && x.Price >= filter.MinimumPrice && x.Price <= filter.MaximumPrice).ToList();
            }
            #endregion
            ...
            if (filter.BedQuantity == null
                    ...
                    && filter.MinimumPrice == null)
            {
                return vm.Where(x => x.BathroomQuantity == filter.BathQuantity && x.Price <= filter.MaximumPrice).ToList();
            }
            if (filter.BedQuantity == null
                && filter.BathQuantity != null
                && filter.MaximumPrice == null
                && filter.MinimumPrice != null)
            {
                return vm.Where(x => x.BathroomQuantity == filter.BathQuantity && filter.MinimumPrice >= x.Price).ToList();
            }
            ...
            if (filter.BedQuantity == null
                ...
                && filter.MinimumPrice != null)
            {
                return vm.Where(x => x.BathroomQuantity == filter.BathQuantity && (x.Price >= filter.MinimumPrice && x.Price <= filter.MaximumPrice)).ToList();
            }
            ...
            if (filter.BedQuantity == null
                ...
                && filter.MinimumPrice != null)
            {
                return vm.Where(x => (x.Price >= filter.MinimumPrice && x.Price <= filter.MaximumPrice)).ToList();
            }
            if (filter.BedQuantity != null
                && filter.BathQuantity != null
                && filter.MaximumPrice != null
                && filter.MinimumPrice == null)
            {
                return vm.Where(x => x.BedRoomQuantity==filter.BedQuantity&&x.BathroomQuantity==filter.BathQuantity&&x.Price <= filter.MaximumPrice).ToList();
            }
            if (filter.BedQuantity != null
                && filter.BathQuantity != null
                && filter.MaximumPrice == null
                && filter.MinimumPrice != null)
            {
                return vm.Where(x => x.BathroomQuantity==filter.BathQuantity&&x.BedRoomQuantity==filter.BedQuantity&&x.Price >= filter.MinimumPrice).ToList();
            }
            ...
            if (filter.BedQuantity != null
                ...
                && filter.MinimumPrice == null)
            {
                return vm.Where(x => x.BathroomQuantity == filter.BathQuantity && x.BedRoomQuantity == filter.BedQuantity).ToList();
            }
            else {
